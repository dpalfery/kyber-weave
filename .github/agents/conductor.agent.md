...
7. With the collection empty, enqueue one `code-reviewer` review over the objective's accumulated change. The five-cycle cap in the `dp-code-reviewer` skill governs that loop. The objective is done only when it reaches `APPROVE` and all contract tests are `GREEN`.
