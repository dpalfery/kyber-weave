...
1. When a dev worker claims a task complete, **immediately release the worker to pull the next ready task**, then gate scope before choosing a reviewer. Development of one task and review of another run at the same time — a worker never sits idle waiting on a review.
   ...
   - A change whose concerns include security, authorization, performance, blast radius, duplication, analyzer triage, dependency supply chain, or coverage-based test adequacy skips `task-reviewer` and goes to `code-reviewer` (or `architect` when the scope needs redesign).
   - Otherwise enqueue a `task-reviewer` task. The invocation carries the objective, the acceptance criteria, the worker's completion digest, and **the pass number** (1 or 2).
2. `task-reviewer` returns one of two results, and never a verdict:
   ...
   - **FAIL** (ordinary, fixable) → create a **rework item** carrying the fix list verbatim plus the original task's files/symbols and acceptance criteria, and place it back on the ready queue for that agent type. This is the coding agent's feedback round, and it gets two of them.
