...
2. `task-reviewer` returns one of two results, and never a verdict:
   ...
   - **FAIL** (ordinary, fixable) → create a **rework item** carrying the fix list verbatim plus the original task's files/symbols, the Test-contract row, and acceptance criteria, and place it back on the ready queue for that agent type. This is the coding agent's feedback round, and it gets two of them.
...
4. A reworked task re-enters step 1 at the next pass number. **Before pass 2, the worker or `test-dev` must regenerate fresh RED/GREEN evidence against the current tree** — prior evidence is not reused verbatim. **A `FAIL` on pass 2 ends the fast loop:** enqueue `code-reviewer` on that one task for pass 3 and report the verdict it returns. There is no pass 4 and no return to `task-reviewer` — the escape hatch is a better reviewer, not another cycle of the same one. Escalate immediately, without finishing the ladder, on a critical security or safety finding. A `NEEDS_HUMAN` verdict is a terminal human handoff: stop immediately — do not enter findings collection and do not continue the automated loop.
5. **Findings collection.** Residual findings from task-level `code-reviewer` reviews — pass 3 or direct — anything the task did not resolve on `REQUEST_CHANGES`, excluding `NEEDS_HUMAN` — go into a per-objective findings collection you track through task state. You hold no write capability, and a review finding does not by itself authorize a repository change, so this is tracked work rather than a file or a todo document.
6. **Drain the collection before the council.** Once every task has completed review — through the ladder or a direct path — route **all** residual findings in the collection through `architect` for a solution and a plan before the objective's review. That plan enters the approval gate in §2 like any other — a `Draft` plan is not executable, so stop and ask. Once approved, route its tasks to workers; they re-enter the ladder at pass 1, and the collection is drained again on the way out.
7. With the collection empty, enqueue one `code-reviewer` review over the objective's accumulated change. The three-cycle cap in the `dp-code-reviewer` skill governs that loop. **`REQUEST_CHANGES` routes findings through that skill's remediation loop** — back to the owning workers for fixes, then a verifier-mode re-review — **until the objective reaches `APPROVE` or the loop terminates per that skill's escalation rules, which is a terminal failure:** stop and report; do not start another automated cycle. A `NEEDS_HUMAN` here is also a terminal human handoff. The objective is done only when it reaches `APPROVE` and all contract tests are `GREEN`.
