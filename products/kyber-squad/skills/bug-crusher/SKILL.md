...
4. **Review — always.** Gate scope before the fast pass: if the change involves security, authorization, performance, blast radius, duplication, analyzer triage, dependency supply chain, or coverage-based test adequacy, hand it to `code-reviewer` (or `architect` when redesign is needed) instead of `task-reviewer`. Otherwise spawn `task-reviewer` on the change, including fixes you applied yourself. Your own edits get *more* scrutiny, not less: you made them without reading the surrounding code, on the investigator's word. A `FAIL` sends the work back with the fix list attached; you get two of those passes. A council-only escalation (`ESCALATION: council-only`), or a `FAIL` on the second pass, hands the change to `code-reviewer` for a full pass rather than a third fast one — do not create a worker rework item for a council-only escalation. Escalate to `architect` if `code-reviewer` does not return `APPROVE`, because a fix that cannot survive review was never trivial.
