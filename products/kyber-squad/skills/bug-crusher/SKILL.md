...
5. **Execute the plan.** Route each task to its specialist by matching against the live agent descriptions, run independent tasks concurrently, and pipeline `task-reviewer` behind each completion so a worker is never idle waiting on a review. `task-reviewer` is a completion audit — it asks whether the task got done and whether the worker's claims hold, in at most two passes. Judgement about the quality of the code belongs to `code-reviewer` over the accumulated change, not to the fast pass. Same rules as `conductor` §3–4 — read that skill if the escalated work grows past a handful of tasks, or hand it to `conductor` outright if the human agrees the scope has changed into a feature.
