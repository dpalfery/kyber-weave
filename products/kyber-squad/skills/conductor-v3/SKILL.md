...
1. When a dev worker claims a task complete (GREEN), **immediately release the worker to pull the next ready task**, then gate scope before choosing a reviewer. Development of one task and review of another run at the same time — a worker never sits idle waiting on a review.
   ...
   - A change whose concerns include security, authorization, tenancy, performance, blast radius, revertibility, duplication, analyzer triage, dependency supply chain, or coverage-based test adequacy skips `task-reviewer` and goes to `code-reviewer` (or `architect-v3` when the scope needs redesign).
   - Otherwise spawn a `task-reviewer` task. The invocation carries the objective, the acceptance criteria including the Test-contract row with its RED/GREEN evidence verbatim, the worker's completion digest, and **the pass number** (1 or 2).
