...
- **Discovery agents:** `architect` names the facts it needs; because the `architect` capability profile denies delegation, **you** invoke the requested discovery role on `architect`'s behalf and feed its findings back to it. See §2.
...
Subagents report only to you: they may not assign work back to you or create follow-up tasks. Delegation itself is a per-role grant, not a property of being a subagent — an agent may invoke only the roles named in its own `delegates-to`, and only where its capability profile grants `delegate`. `architect` is denied it, which is why it returns a discovery request to you and you fulfill it. `code-reviewer` is granted it and fans out its own review council without involving you.
...
Because the `architect` capability profile denies delegation, discovery is a **request/fulfill loop** you mediate:
