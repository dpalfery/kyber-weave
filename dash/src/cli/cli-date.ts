...
import { toDateString } from '../ingest/daily-cache.js'
...
export function toPeriod(s: string): Period {
  try {
    ...
    process.stderr.write(
      `kyberdash: unknown period "${s}". Valid values: ${VALID_PERIODS.join(', ')}.\n`
    )
    process.exit(1)
  }
}
...
export function getDateRange(period: string): { range: DateRange; label: string } {
  ...
  switch (period) {
    ...
    default: {
      process.stderr.write(
        `kyberdash: unknown period "${period}". Valid values: today, week, 30days, month, all, lifetime.\n`
      )
      process.exit(1)
    }
  }
