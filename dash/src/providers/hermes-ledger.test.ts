...
import { aggregateProjectsIntoDays, dateKey } from '../metrics/day-aggregator.js'
import { behavioralTurnCount } from '../metrics/behavioral-weight.js'
import {
  getHermesCursor,
...
skipUnlessSqlite('hermes post-finalization ledger proofs', () => {
  it('P1: sealed day D keeps 100/$0.10/1/1; D+1 is +50/$0.05/0/0', async () => {
    ...
    const { ensureCacheHydrated, loadDailyCache } = await import('../ingest/daily-cache.js')
  ...
  it('P5: seed from a cached lifetime call does not dump lifetime at now', async () => {
    ...
    const parser = await loadParser()
    const ledgerMod = await import('../ingest/hermes-session-ledger.js')
    parser.clearSessionCache()
