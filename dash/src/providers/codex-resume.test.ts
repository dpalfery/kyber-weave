...
vi.mock('../ingest/fs-utils.js', async (importOriginal) => {
...
import { clearCodexMemCaches, flushCodexCache, withCodexCacheDirectory } from '../ingest/codex-cache.js'
...
describe('codex incremental resume', () => {
  ...
  it('falls back to a full re-parse when the stored resume state is unusable', async () => {
    ...
    const { codexCacheFileName } = await import('../ingest/codex-cache.js')
    ...
    const { clearCodexMemCaches } = await import('../ingest/codex-cache.js')
    clearCodexMemCaches()
