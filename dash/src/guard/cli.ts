...
async function doStatus(): Promise<void> {
  ...
  if (found.length === 0) {
    console.log(`    installed:  nowhere (run: ${resolveCliName()} guard install)`)
  } else {
  ...
  if (!flags) {
    console.log(`    flags:      none (run: ${resolveCliName()} guard refresh)`)
  } else {
...
async function doAllow(sessionId: string | undefined): Promise<void> {
  ...
  if (!id) {
    console.error(`  No active guard session found. Pass the session id: ${resolveCliName()} guard allow <session-id>.`)
    process.exitCode = 1
    return
  }
  await writeAllow(id)
