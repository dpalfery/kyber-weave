...
import { mapWithConcurrency } from '../ingest/fs-utils.js'
import type { FileFingerprint } from '../ingest/session-cache.js'
import { fingerprintFile as upstreamFingerprintFile, reconcileFile } from '../ingest/session-cache.js'
import type { Provider, SessionSource, ParsedProviderCall } from '../providers/types.js'
...
async function warmClaudeSpecialPath(dependencies: SourceReaderDependencies): Promise<void> {
  ...
  const { loadCache, monthScopeForRange } = await import('../ingest/session-cache.js')
