...
import { parseApiCall, parseAdvisorCalls, compactEntry, parseAllSessions, clearSessionCache } from './parser.js'
import { calculateCost, loadPricing } from '../pricing/models.js'
...
describe('advisor usage end-to-end through parseAllSessions', () => {
  ...
  const savedEnv = { config: process.env['CLAUDE_CONFIG_DIR'], cache: process.env['KYBERDASH_CACHE_DIR'] }
  ...
  afterEach(async () => {
    ...
    if (savedEnv.cache === undefined) delete process.env['KYBERDASH_CACHE_DIR']
    else process.env['KYBERDASH_CACHE_DIR'] = savedEnv.cache
    if (tmpDir) {
