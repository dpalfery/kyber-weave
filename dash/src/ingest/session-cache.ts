...
export async function cleanupOrphanedTempFiles(): Promise<void> {
  ...
  // `status-snapshot.<queryKeyHash>.json.<nonce>.tmp` is swept too, although
  // nothing writes one any more: the status snapshot went with
  // `status --format menubar-json` (R7.6), so these are leftovers from a cache
  // an earlier binary wrote. They sit in this same parent dir, orthogonal to
  // the month-shard layout below, so the shard-dir sweep further down never
  // sees them and an upgrade would otherwise strand them.
  try {
    for (const entry of await readdir(parent)) {
      if (!entry.endsWith('.tmp')) continue
      if (!/^session-cache\.v\d+\.json\./.test(entry) && !entry.startsWith('status-snapshot.')) continue
