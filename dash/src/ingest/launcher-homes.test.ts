...
import { collectLauncherNotes, isNestedLauncherCodexHome } from './launcher-homes.js'
import { collectDoctorReport, renderDoctorTable } from '../cli/doctor.js'
import { createCodexProvider } from '../providers/codex.js'
import { emptyCache } from './session-cache.js'
...
describe('Codex discover overlap-only nest filter', () => {
  ...
  it('exported no-arg singleton dedups production-size session_meta first lines', async () => {
    ...
    vi.resetModules()
    try {
      const { createCodexProvider: noArgFactory, codex } = await import('../providers/codex.js')
