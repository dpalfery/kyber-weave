...
describe('provider env declarations (#920)', () => {
  it('every process.env read in src/providers is declared for the provider(s) it serves', () => {
    ...
    for (const entry of readdirSync(PROVIDERS_DIR, { withFileTypes: true })) {
      // The walk covers provider modules; the colocated *.test.ts files of the
      // same directory read env vars only to stage fixtures.
      if (!entry.isFile() || entry.name.endsWith('.test.ts') || !entry.name.endsWith('.ts')) continue
