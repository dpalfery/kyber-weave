...
internal static class SquadCommandComposition
{
    ...
    /// files. Copilot, Cursor, Claude, Codex, OpenCode, and Factory are native; Antigravity is
    /// fallback role-skill lowering to <c>.agents/skills/</c>. Every other approved target
    /// fails closed with a pointer to its <c>docs/todo/</c> entry rather than being silently
