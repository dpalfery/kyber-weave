...
public sealed class SquadInstallSettings : CommandSettings
{
    ...
    /// <summary>Harness target(s) to deploy: codex, cursor, claude, copilot, opencode, kilo, antigravity, warp, factory, pi, all.</summary>
    [CommandOption("-t|--target <TARGETS>")]
    [Description("Harness target(s) to deploy: codex, cursor, claude, copilot, opencode, kilo, antigravity, warp, factory, pi, all.")]
    [SuppressMessage(
...
public sealed class SquadUpdateSettings : CommandSettings
{
    ...
    [CommandOption("-t|--target <TARGETS>")]
    [Description("Harness target(s) to update: codex, cursor, claude, copilot, opencode, kilo, antigravity, warp, factory, pi, all.")]
    [SuppressMessage(
        "Performance", "CA1819:Properties should not return arrays",
        Justification = "Spectre.Console.Cli binds repeated options to arrays.")]
    public string[] Targets { get; set; } = [];
