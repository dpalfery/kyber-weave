...
public sealed class SquadUpdateCommand : Command<SquadUpdateSettings>
{
    ...
    public override int Execute(CommandContext context, SquadUpdateSettings settings)
    {
        ...
        AnsiConsole.MarkupLine("[red]kyber-weave: error: Gate G1: qualified upstream APM toolchain is required for update.[/]");
        AnsiConsole.MarkupLine("Upstream toolchain qualification is pending. Update fails closed before writing target files.");
        return 1;
    }
}
