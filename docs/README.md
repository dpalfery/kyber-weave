...
- [Kyber-Squad](kyber-squad/README.md) — Why and how to govern a ten-target harness catalog; six targets render today with rollback
...
The product declares ten harness targets; six are implemented and registered today
(`copilot`, `cursor`, `claude`, `codex`, `antigravity`, and `pi`), while `opencode`, `kilo`, `warp`,
