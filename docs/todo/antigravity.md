---
id: todo/antigravity
title: Add a native Antigravity renderer to Kyber-Squad
doc-type: todo
...
# Add a native Antigravity renderer to Kyber-Squad
...
`squad install --target antigravity` fails today, in preflight, before any network call:
...
**The skill path above is specifically flagged as unverified, not just unconfirmed.** `FakeSquadRenderer` maps Antigravity to the singular `.agent/skills/`, but this repository's own `apm.yml` comment and AGENTS.md describe the general convention as the plural `.agents/skills/` (and this repo's own skill distribution target, `agent-skills`, compiles to `.agents/skills/`). Confirm which one Antigravity actually reads before writing a renderer against either. Same fallback lowering rules as Gemini apply otherwise.
