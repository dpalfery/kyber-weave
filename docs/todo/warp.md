...
**Fallback (role-skill lowering) target.** No native agent primitive is assumed. All 22 canonical agents lower to skills at `.warp/skills/<name>/SKILL.md` per the role-skill rules in `products/kyber-squad/profiles/fallbacks.yml` — see the target-specific note below for the exact projection rules.
