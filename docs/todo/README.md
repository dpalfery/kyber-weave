...
| 8 | [stale-refresh-run-rows.md](stale-refresh-run-rows.md) | KyberDash | `refresh_run` rows stay `running` forever after their refresh process dies (resolved in PR #117 via automatic reconciliation before subsequent refreshes start). |
