---
id: todo/codex
title: Add a native Codex renderer to Kyber-Squad
doc-type: todo
...
# Add a native Codex renderer to Kyber-Squad
...
`squad install --target codex` fails today, in preflight, before any network call:
...
**Native agent target.** Canonical agents render as this harness's own native agent primitive (a `.toml` file, not Markdown) at `.codex/agents/<name>.toml`. Canonical skills render as harness skills at `.codex/skills/<name>/SKILL.md`, except `conductor` and `conductor-v3` — those are native primary agents on every native target and their skill projection is suppressed (the single-projection rule `CopilotRenderer` also follows).

## What is known (from the canonical source and the codebase)

- Strong detection marker: `.codex/`
- Alias(es): none
- The 23 canonical agents and 25 canonical skills this renderer must cover live under
...
FakeSquadRenderer's placeholder Codex output is TOML (`name = "..."` / `description = "..."` / `instructions = """..."""`) — unverified against Codex's real schema. Confirm the actual key names, whether `instructions` is the right key for the body, and whether Codex has anything resembling Copilot's `tools`/model-selection fields before assuming this shape.
...
- Reuse the pattern in `tests/KyberWeave.Tests/SquadRenderingContractTests.cs`: render the
  ...
  checking.
- Confirm `kyber-weave squad install --target codex --dry-run` plans a file for every
...
- Confirm `kyber-weave squad doctor` reports `codex` under renderers available, not
  pending.
