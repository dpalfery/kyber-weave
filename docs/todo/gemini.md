---
id: todo/gemini
title: Add a native Gemini renderer to Kyber-Squad
doc-type: todo
...
# Add a native Gemini renderer to Kyber-Squad
...
`squad install --target gemini` fails today, in preflight, before any network call:
...
**Fallback (role-skill lowering) target.** No native agent primitive is assumed. All 23 canonical agents lower to skills at `.gemini/skills/<name>/SKILL.md` per the role-skill rules in `products/kyber-squad/profiles/fallbacks.yml` — see the target-specific note below for the exact projection rules.
