---
id: todo/cursor
title: Add a native Cursor renderer to Kyber-Squad
doc-type: todo
...
# Add a native Cursor renderer to Kyber-Squad
...
`squad install --target cursor` fails today, in preflight, before any network call:
...
Cursor's own skills convention already appears directly in this repository's `apm.yml` comment (`.cursor/skills/` is one of the per-client paths APM's `--legacy-skill-paths` flag targets), which is weak corroboration for the skill path but says nothing about the agent file format itself.
...
- Reuse the pattern in `tests/KyberWeave.Tests/SquadRenderingContractTests.cs`: render the
  ...
  checking.
- Confirm `kyber-weave squad install --target cursor --dry-run` plans a file for every
...
- Confirm `kyber-weave squad doctor` reports `cursor` under renderers available, not
  pending.
