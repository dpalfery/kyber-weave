---
id: archive/todo/antigravity-capability-verification-evidence
...
Evidence for task C0 of the completed [Pi thinking and Antigravity native agents plan](../plans/2026-09-21-pi-thinking-and-antigravity-native-agents.md). It is a record, not remaining work.
...
| 1. Tool vocabulary mapping — live capability | Mixed, per row (§1) | `filesystem.read`, `filesystem.write`/`write_to_file`, `process.execute` CONFIRMED with attempt-and-verify + agy-record proof; `filesystem.search`/`list_dir` CONFIRMED on declared/self-report-consistency; five rows (`grep_search`, `replace_file_content`, `multi_replace_file_content`, `network.read`, `delegate`) remain INCONCLUSIVE for lack of a live exercise — candidates for the next live-verification pass, not blockers for C1's already-covered rows |
