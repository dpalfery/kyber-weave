...
`antigravity`, `opencode`, and `factory`. The product declares two more targets—`kilo` and
`warp`—but requesting either of them currently fails renderer-coverage preflight before a
deployment starts.
