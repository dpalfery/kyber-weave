...
for six currently implemented and registered renderers: `copilot`, `cursor`, `claude`, `codex`,
`antigravity`, and `pi`. The product declares four more targets—`opencode`, `kilo`, `warp`, and
`factory`—that have no renderer and currently fail renderer-coverage preflight before a
deployment starts.
