...
harness targets. Seven renderers are implemented and registered (`copilot`, `cursor`, `claude`,
`codex`, `antigravity`, `opencode`, and `warp`); `kilo` and `factory` fail renderer-coverage
