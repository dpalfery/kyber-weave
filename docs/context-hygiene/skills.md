...
harness targets. Eight renderers are implemented and registered (`copilot`, `cursor`, `claude`,
`codex`, `antigravity`, `opencode`, `kilo`, and `pi`); `warp` and `factory` fail renderer-coverage
