...
`codex` (native), `antigravity` (fallback role-skill lowering to `.agents/skills/`), `opencode` (native), and `factory` (native) have renderers. Requesting any other target fails before the release is even downloaded,
