...
| `permission-not-expressible` | A non-deny capability decision cannot be expressed in the target's native permission model without inventing an unverified mapping. | Factory omits permission-equivalent frontmatter and records this code for every agent whose profile has any non-deny decision. |
...
| **Factory Droids** | Native `.factory/` | Implemented and registered | Supported | Not lowered | Permission-not-expressible (unverified mapping omitted) |
