...
|---|---|
| **KS-001** | **Canonical Source Governance**: Maintain exactly 20 canonical agent instruction bodies and 25 canonical skill source directories under `products/kyber-squad/`. Generated role-skill projections do not alter the source inventory, and generated APM, plugin, or harness trees are never tracked in source control. |
...
| **KS-005** | **Version Lockstep & Toolchain Attestation**: Enforce exact version equality across the CLI, Squad release asset, and MCP server, alongside qualifying APM toolchain verification. Verify all release assets against published SHA-256 checksums without installing external dependencies as side effects. |
